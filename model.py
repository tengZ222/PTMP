...
class TrajectoryModel(nn.Module):
    ...
    def __init__(self, in_size, obs_len, pred_len, embed_size, enc_num_layers, int_num_layers_list, heads, forward_expansion):
        ...
        self.debedding = nn.Linear(embed_size*2,embed_size)
    ...
    def spatial_interaction(self, ped, neis, mask):
        ...
        B, K, embed_size = ped.shape
        N = neis.shape[1]
        neis = neis.reshape(neis.shape[0], neis.shape[1], -1)  # [B N obs_len*2]
        nei_embeddings = self.nei_embedding(neis)  # [B N embed_size]
        ped = ped.unsqueeze(2).expand(B, K, N, embed_size).contiguous()
        ped = ped.view(B, K * N, embed_size)
        neis = nei_embeddings.unsqueeze(1).repeat(1, K, 1, 1).contiguous()
        neis = neis.view(B, K * N, embed_size)
        gat_input = torch.cat([ped, neis], dim=2)#[B K embed_size*2]
        gat_input = self.debedding(gat_input)
        neis = self.GAT_Encoder(gat_input)#nei_embeddings
        neis = neis.view(B, K, N, embed_size)
        #neis = neis.sum(dim=2)
        neis = neis.mean(dim=2)
        mask = mask[:, 0:1].repeat(1, K, 1)  # [B K N]
        int_feat = self.social_decoder(neis, nei_embeddings, mask)  # [B K embed_size]
