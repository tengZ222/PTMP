...
parser.add_argument("--hp_config", type=str, default='config/sdd.py', help='hyper-parameter')
...
#model.load_state_dict(torch.load(r'C:\Users\tengz.DESKTOP-HQJ3RLT\PycharmProjects\pythonProject\-Tengz-2024\checkpoint\univ\best.pth'))
model = model.cuda()
...
def train(epoch, model, reg_criterion, cls_criterion, optimizer, train_dataloader, motion_modes):
    ...
    for i, (ped, neis, mask) in enumerate(train_dataloader):
        ...
        scale = torch.randn(ped.shape[0])*0.05+1 #[B]
        scale = scale.cuda()
        ...
        with torch.no_grad():
            soft_label, closest_mode_indices = get_cls_label(gt, motion_modes)# [B K]  [B]
...
def vis_predicted_trajectories(obs_traj, gt, pred_trajs, pred_probabilities, min_index):
    ...
    for i in range(obs_traj.shape[0]):
        ...
        plt.plot(obs_x, obs_y, ls='-.', color='green')
        plt.plot(gt_x, gt_y, ls='-.', color='blue')

        # 绘制真实轨迹终点的箭头
        plt.quiver(gt_x[-2], gt_y[-2], gt_x[-1] - gt_x[-2], gt_y[-1] - gt_y[-2],
        ...
        for j in range(curr_preds.shape[0]):
            ...
            if j == curr_min_index:
                ...
                plt.quiver(pred_x[-2], pred_y[-2], pred_x[-1] - pred_x[-2], pred_y[-1] - pred_y[-2],
                           angles='xy', scale_units='xy', scale=1, color='red', width=0.005)
            else:
                plt.plot(pred_x, pred_y, ls='-.', lw=0.5, color='orange')
                plt.quiver(pred_x[-2], pred_y[-2], pred_x[-1] - pred_x[-2], pred_y[-1] - pred_y[-2],
...
def test(model, test_dataloader, motion_modes):
    ...
    for (ped, neis, mask) in test_dataloader:
        ...
        with torch.no_grad():
            ...
            pred_trajs, scores = model(ped_obs, neis_obs, motion_modes, mask, None, test=True)#[B num_k pred_size*2] [B K]
            top_k_scores = torch.topk(scores, k=20, dim=-1).values
            top_k_scores = F.softmax(top_k_scores, dim=-1)
            # pred_trajs =[B num_k pred_size*2]
            pred_trajs = pred_trajs.reshape(pred_trajs.shape[0], pred_trajs.shape[1], gt.shape[1], 2) #[B num_k pred_size,2]
            gt_ = gt.unsqueeze(1)  #[B, 1, pred_len, 2]
            norm_ = torch.norm(pred_trajs - gt_, p=2, dim=-1)#[B, num_k, pred_len]表示在每个批次、每种模式下，每个时间步的预测位置与真实位置之间的距离
            ade_ = torch.mean(norm_, dim=-1)  #[B, num_k]
            fde_ = norm_[:, :, -1] #[B, num_k]
            min_ade, min_ade_index = torch.min(ade_, dim=-1) #[B] [B]
    ...
    average_inference_time = total_inference_time / len(test_dataloader)
    ade = ade / num_traj
    fde = fde / num_traj
...
for ep in range(hp_config.epoch):
    ...
    print('epoch:', ep, 'ade:', ade, 'fde:', fde, 'min_ade:', min_ade, 'min_fde:', min_fde, 'num_traj:', num_traj,
          "min_fde_epoch:", min_fde_epoch,'current_time:', current_time)